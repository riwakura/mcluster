...
#pragma omp parallel
	...
	if (nbin) {
		...
		if (pairing) {
			...
			else if (pairing == 3) printf("\nApplying Sana et al. (2012) mass ratio distribution for stars with masses > %.1f Msun.\n",msort);
			else if (pairing == 4) printf("\nApplying AIC/ESCN-boost mass ratio distributiuon");
	...
	for (i=0; i < nbin; i++) {
		...
		do {
			...
			if (((m1*M>=msort) || (m2*M>=msort)) && (OBperiods)) {
				...
				abin = pow((m1+m2)*M*P*P,(1.0/3.0));//AU				abin /= 206264.806;//pc
...
*/
    ...
    // Ryutaro Iwakura add pairing=4 for AIC/ESCN-boost mass ratio distribution
	// for 5 Msun <= m < 8 Msun stars, AIC-boost sampling
    // for 8 Msun <= m < 10 Msun stars, ECSN-boost sampling
	// for m >= 10 Msun stars, Sana+2012's uniform mass distribution is used: f(q) ~ q^n (n=0; 0.1<=q<=1.0), same as pairing=3
	// Sampling order: ECSN-boost -> AIC-boost -> > Others (10 Msun)
	// when using this option, do not change the sorting criterion mass so far; fix as msort = 5.0
    } else if (pairing==4) {
		// Mask to show which mass is already used, 1 means used
		int *mask = (int*)calloc(N,sizeof(int));
		if (mask == NULL) {
		...
		// allocate N*2 allay to store mass data which are not used during m>5 Msun sorting process
		double **mmrand = (double**)calloc(N,sizeof(double *));
		for (j=0;j<N;j++){
			mmrand[j] = (double *)calloc(2,sizeof(double));
			if (mmrand[j] == NULL) {
		...
		// ECSN-boosting samplling for 8 Msun <= m < 10 Msun stars
		// primary star for ECSN-boost sampling
		for (i=0;i<N;i++){
		    if (mask[i]==0 && masses[i][0] >= 8.0 && masses[i][0] < 10.0){
    			star_temp[j][0] = star[(int) masses[i][1]][0];
    			star_temp[j][1] = star[(int) masses[i][1]][1];
    			star_temp[j][2] = star[(int) masses[i][1]][2];
    			star_temp[j][3] = star[(int) masses[i][1]][3];
		...
		// AIC-boosting samplling for 5 Msun <= m < 8 Msun stars
		// Primary star for AIC-boost sampling, (5 Msun <= m < 8 Msun)
		for (i=0;i<N;i++){
		    if (mask[i]==0 && masses[i][0] >= msort && masses[i][0] < 8.0){
				star_temp[j][0] = star[(int) masses[i][1]][0];
				star_temp[j][1] = star[(int) masses[i][1]][1];
				star_temp[j][2] = star[(int) masses[i][1]][2];
				star_temp[j][3] = star[(int) masses[i][1]][3];
        ...
        // Sana+2012 property for m > 10 Msun stars
        // Primary star for Sana+2012 uniform distribution
		for (i=0;i<N;i++){
			if(mask[i]==0 && masses[i][0] >= 10.0) {
				star_temp[j][0] = star[(int) masses[i][1]][0];
				star_temp[j][1] = star[(int) masses[i][1]][1];
				star_temp[j][2] = star[(int) masses[i][1]][2];
				star_temp[j][3] = star[(int) masses[i][1]][3];
		...
		// Store index for m < 5 Msun stars
		// these stars are sampled using Kroupa (1995) distribution
		for (i=0;i<N;i++){
		    if (mask[i]==0){
				mmrand[ileft][0] = drand48();
				mmrand[ileft][1] = masses[i][1];
...
void help(double msort) {
	...
	printf("       -p <0|1|2|3|4> (binary mass pairing:                            \n");
    printf("                   0= random,                                        \n");
    ...
    printf("       -E <0|1|2|3>   (binary semi-major axis/eccentricity distribution for M>%.1f Msun:\n",msort);
